"""Train Step-SFT v2.1-clean: Mix original SFT data with CLEAN read-to-edit data.

Base: Step-SFT v2 checkpoint
Data: 70% step_sft_clean + 30% read_to_edit_clean_train_only
Only train-split tasks, no test/val/heldout contamination.
"""
...
def setup_logging(output_dir: Path) -> logging.Logger:
    logger = logging.getLogger("train_v21_clean")
...
def main():
    ...
    parser.add_argument("--read_to_edit_data", type=str, default="outputs/data/read_to_edit_step_sft_clean_train_only.jsonl")
    parser.add_argument("--output_dir", type=str, default="outputs/models/3b_step_sft_v21_clean")
    ...
    logger.info(f"Clean read-to-edit samples: {len(r2e_samples)}")

    # Verify r2e is clean
    heldout = {f"bugfix_{i}" for i in range(201, 251)}
    r2e_tasks = {s["task_id"] for s in r2e_samples}
    test_path = root / "outputs" / "data" / "splits" / "test_tasks.txt"
    test_tasks = {l.strip() for l in test_path.read_text(encoding="utf-8").splitlines() if l.strip()}
    assert len(r2e_tasks & heldout) == 0, f"Held-out contamination: {r2e_tasks & heldout}"
    assert len(r2e_tasks & test_tasks) == 0, f"Test split contamination: {r2e_tasks & test_tasks}"
    logger.info("R2e data cleanliness verified: no heldout/test contamination")
    ...
    # Load v2 adapter with is_trainable=True
    ...
    # Format data
    # CRITICAL FIX: r2e samples have completion as a separate field.
    # Must append it as the final assistant message before formatting.
    logger.info("Formatting data...")
    formatted_samples = []
    for s in mixed_samples:
        if "messages" in s:
            msgs = list(s["messages"])
            # Append completion as assistant message if not already present
    ...
    training_args = SFTConfig(
        ...
        report_to="none", completion_only_loss=False, max_grad_norm=1.0,
    )
    ...
    stats = {
        "base_model": args.base_model, "sft_adapter": args.sft_adapter,
        "original_samples": len(original_samples), "r2e_samples": len(r2e_samples),
        "mixed_samples": len(mixed_samples), "mix_ratio": args.mix_ratio,
