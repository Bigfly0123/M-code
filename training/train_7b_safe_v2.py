"""7B-safe-v2 Step-SFT (EOS + completion_loss=False): fixed EOS + completion_only_loss=False.

Key fixes vs old 7B training:
1. Append eos_token to completion
2. completion_only_loss=False (only train on action JSON + EOS)
...
def main():
    ...
    output_dir = root / "outputs" / "models" / "7b_step_sft_v2_safe_v2"
    ...
    # Train with completion_only_loss=False
    training_args = SFTConfig(
        ...
        report_to="none",
        completion_only_loss=False,  # KEY FIX: only train on completion
        max_grad_norm=1.0,
    )

    trainer = SFTTrainer(model=model, args=training_args, train_dataset=dataset, processing_class=tokenizer)

    print("Starting 7B-safe-v2 Step-SFT (EOS + completion_loss=False) training...")
    trainer.train()
