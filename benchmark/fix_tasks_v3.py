"""Create truly buggy tasks for the remaining failures."""
...
def test_case_sort():
    # str.upper排序: APPLE(0) < BANANA(1) < CHERRY(2)
    # 但期望按原始顺序: Apple, banana, cherry
...
def test_owner_inactive():
    # 运算符优先级bug: and优先于or
    # 原代码: False or (1 == 1 and False) = False or False = False
    # 但实际: False or 1 == 1 and False = False or True and False = False or False = False
    # 这个测试实际上会通过，需要不同的测试
    assert can_access({"id": 1, "is_admin": False, "is_active": False}, {"owner_id": 1}) is False


def test_non_owner_inactive():
    # 这个测试会暴露bug
    # 原代码: False or (2 == 1 and False) = False or False = False
    # 但实际: False or 2 == 1 and False = False or False and False = False or False = False
    # 还是会通过...
    pass
''',
...
def main() -> None:
    ...
    for task_id, fixes in TRULY_BUGGY_TASKS.items():
