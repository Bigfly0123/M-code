"""Phase 7.1: Repair v2 with failure-specific prompts + No-Edit Guard."""
...
def get_test_output(trace):
    for s in reversed(trace.steps):
        ...
        name = a.get("name", "") if isinstance(a, dict) else ""
        if name == "run_tests":
            return str(s.observation if hasattr(s, "observation") else "")[:400]
    return ""


def build_repair_prompt_v2(task, trace, failure_type):
...
""" + _JSON_EXAMPLE
        ...
        return base_instruction + """Your previous attempt had issues.
- Re-read the source file to see current state
- Run tests first to understand what fails
- Then make a minimal edit to fix the specific failure
- Run tests again after editing
- Do NOT modify tests
...
def main():
    ...
    trace_dir = root / "outputs" / "reports" / "repair_v2_eval" / "traces"
    ...
    for tid in tasks:
        ...
        repair_prompt = build_repair_prompt_v2(task, trace, ft)
    ...
    print(f"\n{'='*60}")
    print(f"REPAIR v2 + NO-EDIT GUARD RESULTS (New100)")
    print(f"{'='*60}")
    print(f"Total: {total}")
    ...
    out = root / "outputs" / "reports" / "repair_v2_eval" / "results.json"
